...
export function Episodes({ }) {
    ...
    const [totalEpisodes, setTotalEpisodes] = useState(1000); // Add state for total episodes
    ...
    return (
        <div>
            <Search />
            <Stream episodeId={episodeId} totalEpisodes={totalEpisodes} id={id} setEpisodeId={handleOnClick} />
            ...
            <div className="buttonBox">
                ...
                <div className="buttonContainer">
                    {[...Array(totalEpisodes)].map((_, index) => (
                        <div key={index}>
                            <Link to={`/watch/${id}/`} className={`epButton ${episodeId === `${id}-episode-${index + 1}` ? 'selected' : ''}`} onClick={() => handleOnClick(`${id}-episode-${index + 1}`)}>E{index + 1} </Link>
                        </div>
                    ))}
                </div>
            </div>
